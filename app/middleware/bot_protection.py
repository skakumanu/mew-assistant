...
class BotProtectionMiddleware(BaseHTTPMiddleware):
    ...
    async def _check_suspicious_content(self, request: Request) -> bool:
        ...
        # Check URL path
        if any(re.search(pattern, request.url.path, re.IGNORECASE) 
...
class CaptchaVerifier:
    ...
    def generate_challenge(self, user_id: str) -> dict:
        ...
        self.pending_verifications[challenge_id] = (str(answer), datetime.utcnow() + timedelta(minutes=5))
    ...
    def cleanup_expired(self):
        ...
        now = datetime.utcnow()
        expired = [
            cid for cid, (_, exp) in self.pending_verifications.items()
            if now > exp
        ]
        for cid in expired:
