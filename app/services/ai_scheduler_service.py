...
class AISchedulerService:
        ...
        existing_entries = self.db.query(ScheduleEntry).filter(
            and_(
                ...
                or_(
                    ...
                    and_(ScheduleEntry.start_time >= start, ScheduleEntry.end_time <= end)
                )
            )
        ).all()
    ...
    async def _learn_user_patterns(
        self,
        user_id: int,
        activity_type: str
    ) -> Dict:
        ...
        historical = self.db.query(ScheduleEntry).filter(
            and_(
                ScheduleEntry.user_id == user_id,
