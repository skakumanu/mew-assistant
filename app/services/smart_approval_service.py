...
class SmartApprovalService:
    ...
    def _check_auto_approval_rules(
        ...
        rules = self.db.query(ApprovalRule).filter(
            ApprovalRule.user_id == parent.id,
    ...
    async def _check_learned_patterns(
        ...
        similar_requests = self.db.query(ApprovalRequest).filter(
    ...
    def _calculate_similarity(
        ...
        time_diff = abs(
            req1.proposed_start_time.hour - req2.proposed_start_time.hour
        )
    ...
    def _is_low_risk_routine(self, request: ApprovalRequest) -> bool:
        ...
        low_risk_activities = [
            'homework', 'reading', 'practice', 'break',
            'snack', 'free_play', 'rest'
        ]
    ...
    async def batch_pending_requests(
        ...
        urgent = [r for r in pending if r.priority == RequestPriority.URGENT or
                  self._is_time_sensitive(r)]
        if urgent:
            batches.append({
                'batch_id': 'urgent',
                'requests': urgent,
    ...
    async def suggest_rules_from_history(self, parent: User) -> List[Dict]:
        ...
        short_activities = [r for r in approved if
                           (r.proposed_end_time - r.proposed_start_time).total_seconds() <= 1800]
