...
from ..database.models import CaregiverSummary as SummaryModel
...
class SummaryService:
    ...
    def get_summary(self, summary_id: int) -> Optional[SummaryModel]:
        """Get summary by ID."""
        return self.db.query(SummaryModel).filter(SummaryModel.id == summary_id).first()

    def get_user_summaries(self, user_id: str, limit: int = 50) -> List[SummaryModel]:
        """
        ...
        return (
            ...
            .filter(SummaryModel.user_id == user_id)
    ...
    def _generate_recommendations(self, sessions: List[SessionModel]) -> List[str]:
        ...
        if "scheduling" in type_counts:
            recommendations.append(
                "Try to schedule sessions at consistent times for routine building"
            )
