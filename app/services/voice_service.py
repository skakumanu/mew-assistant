...
from app.database.models import User, VoiceCommand
...
class VoiceService:
    ...
    async def process_voice_command(
        ...
        try:
            ...
            transcription = await self.voice_integration.transcribe_audio(
                audio_data=audio_data, hint_language=hint_language
            )
            ...
            action_result = await self._process_intent(
                transcription=transcription, user_id=user_id, session_id=session_id
            )
    ...
    async def _process_intent(
        self, transcription: VoiceTranscription, user_id: str, session_id: Optional[str]
    ) -> Dict[str, Any]:
        """
        ...
        if intent == "schedule_appointment":
            ...
            return await self._handle_set_reminder(entities=entities, user_id=user_id)
    ...
    async def _handle_schedule_appointment(
        self, entities: Dict[str, Any], user_id: str, session_id: Optional[str]
    ) -> Dict[str, Any]:
