...
def list_all_users(db):
    ...
    for user in users:
        ...
        has_oauth = db.query(FederatedIdentity).filter(FederatedIdentity.user_id == user.id).first() is not None
...
def main():
    ...
    parser.add_argument("--live", action="store_true", help="Actually delete users (default is dry-run)")
